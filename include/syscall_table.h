...
typedef enum {
    SYSCALL_CAT_UNKNOWN = 0,
    SYSCALL_CAT_PROCESS,          /* fork, exec, exit, wait */
    SYSCALL_CAT_FILE,             /* open, read, write, close */
    SYSCALL_CAT_NETWORK,          /* socket, connect, send, recv */
    SYSCALL_CAT_MEMORY,           /* mmap, brk, mprotect */
    SYSCALL_CAT_SIGNAL,           /* kill, sigaction, sigprocmask */
    SYSCALL_CAT_IPC,              /* pipe, shmget, msgget */
    SYSCALL_CAT_TIME,             /* gettimeofday, clock_gettime */
    SYSCALL_CAT_SECURITY,         /* setuid, setgid, prctl */
    SYSCALL_CAT_INFO,             /* uname, getpid, getuid */
    SYSCALL_CAT_IO,               /* ioctl, poll, select */
    SYSCALL_CAT_FILESYSTEM,       /* mount, chroot, chdir */
    SYSCALL_CAT_MISC,             /* Other syscalls */
} sg_syscall_cat_t;
...
typedef struct {
    ...
    sg_syscall_cat_t category;
    int arg_count;                        /* Number of arguments */
    sg_arg_type_t arg_types[MAX_SYSCALL_ARGS];
    bool is_essential;                    /* Required for basic operation */
    bool is_sensitive;                    /* Security-sensitive */
...
/*
 * Clean up syscall table resources
 */
...
/*
 * Check if syscall is in a category
 */
bool sg_syscall_is_category(uint32_t nr, sg_syscall_cat_t cat);

/*
 * Check if syscall is essential (should always be allowed)
 */
bool sg_syscall_is_essential(uint32_t nr);

/*
 * Check if syscall is security-sensitive
 */
 ...
 */
size_t sg_syscall_get_by_category(sg_syscall_cat_t cat, 
                                   uint32_t *out, 
                                   size_t max_count);

/*
 * Get list of essential syscalls
 */
size_t sg_syscall_get_essential(uint32_t *out, size_t max_count);

/*
 * Decode syscall arguments based on type information
 */
...
/*
 * Format syscall for display
 * Returns pointer to static buffer
 */
const char *sg_syscall_format(const sg_syscall_event_t *event);

/*
 * Format syscall arguments for display
 */
int sg_syscall_format_args(const sg_syscall_event_t *event, 
                           char *buf, 
                           size_t len);

/* Common ARM64 Linux syscall numbers */
